...
public class EpisodeApiRestClient
{
    ...
    public List<EpisodeCharactersData.Character> getEpisodeInfo(int episodeId)
    {
        String graphqlQuery = String.format("{ episode(id: %d) { id characters { id } } }", episodeId);
       
        EpisodeCharactersData episodeInfoData = restClient.post()
                .uri("/graphql")
                .header("Content-Type", MediaType.APPLICATION_JSON_VALUE)
                .body("{\"query\":\"" + graphqlQuery + "\"}")
                .retrieve()
        ...
        List<EpisodeCharactersData.Character> characters = Optional.ofNullable(episodeInfoData)
